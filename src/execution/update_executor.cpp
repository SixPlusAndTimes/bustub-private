...
bool UpdateExecutor::Next([[maybe_unused]] Tuple *tuple, RID *rid) { 
  ...
  if (get_tuple_succeed) {
    ...
    bool update_tuple_in_heap_succeed = table_info_->table_->UpdateTuple(updated_tuple, rid_to_be_updated, exec_ctx_->GetTransaction());
    if (update_tuple_in_heap_succeed) {
      // 根新索引
      for(auto& index_info : indexs_info_) {
        Tuple old_key_tuple = tuple_to_be_updated.KeyFromTuple(table_info_->schema_, index_info->key_schema_, index_info->index_->GetKeyAttrs());
        Tuple new_key_tuple = updated_tuple.KeyFromTuple(table_info_->schema_, index_info->key_schema_, index_info->index_->GetKeyAttrs());
        // 先删除然后再插入达到更新的目的
